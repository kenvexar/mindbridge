...
# Discord 設定（実際のボットでテストする場合のみ）
# DISCORD_BOT_TOKEN=your_dev_token
# DISCORD_GUILD_ID=your_test_server_id
# GEMINI_API_KEY=your_dev_key
```

💡 **完全な設定例**: [.env.example](../../.env.example) ファイルを参照してください。
...
│   ├── gemini_client.py    # Google Gemini クライアント
│   ├── note_analyzer.py    # ノート分析と分類
│   ├── url_processor.py    # URL コンテンツ処理
│   ├── mock_processor.py   # モック AI プロセッサー
│   ├── vector_store.py     # ベクトルストア
│   └── models.py           # AI データモデル
├── obsidian/            # Obsidian 統合
│   ├── core/               # コアボルト操作
│   ├── template_system/    # テンプレートシステム
│   ├── search/             # 検索機能
│   ├── backup/             # バックアップ管理
│   ├── analytics/          # ボルト分析
...
│   ├── report_generator.py # 財務レポート
│   ├── reminder_system.py  # 支払いリマインダー
