...
| コンポーネント | 責任 | 技術 |
|---------------|------|------|
| **Discord Client** | ユーザーインターフェース | discord.py 2.5+ |
| **AI Processor** | メッセージの分析・分類 | Google Gemini API (google-genai) |
| **Obsidian Manager** | ノート管理・保存 | aiofiles, YAML 処理 |
| **Task Manager** | タスク管理・進捗追跡 | JSON 永続化, Pydantic モデル |
| **Finance Manager** | 家計管理・支出追跡 | ExpenseManager, SubscriptionManager |
| **Component Manager** | 遅延ロード・キャッシュ管理 | 独自実装 |
| **Stats Commands** | 統計・分析機能 | Pydantic モデル |
| **Security Manager** | 認証情報管理 | Google Secret Manager |
| **Health Server** | ヘルスチェック・監視 | aiohttp サーバー |
...
├── commands/             # コマンドモジュール
│   ├── basic_commands.py    # 基本コマンド (help, status, search, random)
│   ├── stats_commands.py    # 統計コマンド (bot, obsidian, finance, task)
│   ├── task_commands.py     # タスク管理コマンド
│   ├── finance_commands.py  # 家計管理コマンド
...
**特徴:**
- Discord API との直接やり取り (discord.py 2.5+)
- 階層化されたコマンド構造
- 共通エラーハンドリング（ CommandMixin ）
- レスポンスフォーマット
...
│   │   ├── finance_commands.py # 家計管理コマンド（実装済み）
│   │   ├── task_commands.py    # タスク管理コマンド（実装済み）
│   │   └── integration_commands.py # 外部連携コマンド
│   ├── mixins/            # 🔄 再利用可能なミックスイン
...
│
├── finance/               # 🏦 家計管理システム
│   ├── expense_manager.py # 💰 支出・収入管理（実装済み）
│   ├── subscription_manager.py # 🔄 定期購入管理（実装済み）
│   ├── budget_manager.py  # 📊 予算管理（未実装）
│   ├── report_generator.py # 📈 財務レポート生成（未実装）
│   ├── reminder_system.py # ⏰ 支払いリマインダー（未実装）
│   ├── message_handler.py # 📨 メッセージハンドラー（未実装）
│   ├── commands.py        # ⚡ ファイナンスコマンド（未使用）
│   ├── models.py          # 💳 金融データモデル（実装済み）
│   └── __init__.py
│
├── tasks/                 # ✅ タスク管理システム
│   ├── task_manager.py    # 📋 タスク管理（ CRUD 操作、実装済み）
│   ├── schedule_manager.py # 📅 スケジュール管理（未実装）
│   ├── reminder_system.py # 🔔 リマインダー（未実装）
│   ├── report_generator.py # 📊 生産性レポート（未実装）
│   ├── commands.py        # ⚡ タスクコマンド（未使用）
│   ├── models.py          # 📝 タスクデータモデル（実装済み）
│   └── __init__.py
│
