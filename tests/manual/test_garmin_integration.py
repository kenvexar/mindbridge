...
project_root = Path(
    __file__
).parent.parent.parent  # tests/manual/../../ -> project root
...
async def test_garmin_connection():
    ...
    if connection_result["success"]:
        ...
        print(
            f"   ユーザー向けメッセージ: {connection_result.get('user_message', 'エラーが発生しました')}"
        )
        ...
        if not connection_result["authenticated"]:
            ...
            has_password = (
                hasattr(settings, "garmin_password") and settings.garmin_password
            )
    ...
    try:
        ...
        if health_data.has_any_data:
            ...
            if health_data.heart_rate and health_data.heart_rate.resting_heart_rate:
                print(
                    f"      安静時心拍数: {health_data.heart_rate.resting_heart_rate}bpm"
                )
...
async def test_cache_management():
    ...
    try:
        ...
        print(
            f"   クリーンアップ後のキャッシュサイズ: {new_stats.get('cache_size', 0)} ファイル"
        )
