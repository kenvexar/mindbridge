...
async def create_test_audio_files() -> Dict[str, bytes]:
    ...
    try:
        ...
        # フォールバック: 簡単なダミーデータ
        test_files = {
            "dummy_test.mp3": b"dummy audio data for testing"
        }
...
async def test_speech_processor():
    ...
    print(f"  - 利用可能エンジン: {[engine['name'] for engine in processor.transcription_engines]}")
    
    # 2. 使用量統計の確認
    stats = processor.get_usage_stats()
    print(f"\n 📈 API 使用量統計:")
    print(f"  - 月間使用量: {stats['monthly_usage_minutes']:.1f}分 / {stats['monthly_limit_minutes']:.1f}分")
    ...
    for filename, file_data in test_files.items():
        ...
        try:
            ...
            results.append({
                ...
                "transcription": result.transcription.transcript if result.transcription else None,
                "confidence": result.transcription.confidence if result.transcription else 0.0,
                "processing_time": result.processing_time_ms,
                "model_used": result.transcription.model_used if result.transcription else None,
...
async def generate_test_report(results: List[Dict[str, Any]], stats: Dict[str, Any]):
    ...
    print(f"  成功: {successful_tests} ({'100' if total_tests == 0 else successful_tests/total_tests*100:.1f}%)")
    ...
    for i, result in enumerate(results, 1):
        ...
        if result.get('success'):
            ...
            print(f"     文字起こし: {transcript[:50]}{'...' if len(transcript) > 50 else ''}")
...
async def main():
    ...
    try:
        ...
        # 5. 総合結果
        print(f"\n 🎯 総合テスト結果:")
        print(f"  音声処理: ✅ {len([r for r in speech_results if r.get('success')])}/{len(speech_results)} 成功")
        print(f"  Bot 統合: {'✅ 成功' if bot_integration_success else '❌ 失敗'}")
        print(f"  Obsidian 統合: {'✅ 成功' if obsidian_integration_success else '❌ 失敗'}")
        
        overall_success = (
            len([r for r in speech_results if r.get('success')]) > 0 and
            bot_integration_success and
            obsidian_integration_success
        )
