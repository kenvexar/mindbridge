    ...
    try:
        ...
        for i, content in enumerate(test_messages, 1):
            ...
            try:
                ...
                results.append({"message": content, "success": True, "result": result})
                # AIProcessingResult オブジェクトの属性にアクセス
                category_info = (
                    result.category.category.value if result.category else "Unknown"
                )
                print(f"✅ 処理成功: {category_info}")

            except Exception as e:
                results.append({"message": content, "success": False, "error": str(e)})
                print(f"❌ 処理失敗: {e}")
